        ...
        for feature in features_arc:
            ...
            if similarity >= threshold:
                ...
                similar_users.append({
                    ...
                    "matched_models": set([("ArcFace", similarity)]) 
                })
                ...
                if user.id in matched_users:
                    for entry in similar_users:
                        if entry["image"] == user.image:
                            entry["matched_models"].add(("VGG-Face", similarity))
                    ...
                    similar_users.append({
                        ...
                        "model_used": "VGG-Face",
                        "occlusion_percentage": occ_per,
                        "matched_models": set([("VGG-Face", similarity)])  # Using set for unique models
                    })
        ...
        for feature in features_sf:
            ...
            if similarity >= threshold:
                ...
                if user.id in matched_users:
                    for entry in similar_users:
                        if entry["image"] == user.image:
                            entry["matched_models"].add(("SFace", similarity))  # Add to set
                            entry["similarity"] = max(entry["similarity"], similarity)
                else:
                    matched_users.add(user.id)
                    similar_users.append({
                        ...
                        "model_used": "SFace", 
                        "occlusion_percentage": occ_per,
                        "matched_models": set([("SFace", similarity)])  # Using set for unique models
                    })
